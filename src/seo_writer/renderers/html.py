...
.sr-only{left:0;top:0}@media(max-width:600px){.stage-nav{width:calc(100% - 28px);margin-inline:14px}}
""".strip()
    js = """
...
document.getElementById('download-review')?.addEventListener('click',()=>{
 ...
 if(seed.review_type==='opportunity'&&!payload.reviewer.trim()){document.getElementById('opportunity-reviewer')?.reportValidity();return;}
 const decisions=[];document.querySelectorAll('[data-decision]').forEach(row=>{const item={};row.querySelectorAll('[data-key]').forEach(el=>item[el.dataset.key]=el.value);if(seed.review_type!=='opportunity'||item.decision)decisions.push(item)});if(decisions.length)payload.decisions=decisions;
...
""".strip()
    return (
        ...
        f'<button id="download-review" type="button">{_e(context.get("download_label", "Download review JSON"))}</button>'
