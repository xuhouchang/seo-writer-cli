...
The deterministic Phase 1 keyword/SERP/community/LLM providers use bundled mock
fixtures. They demonstrate and test the governance pipeline; they are not live
market research. Production content should be authored by the Agent through
`--from-file` and must use separately verified research. GSC becomes real only
after the customer explicitly configures OAuth, or imports a dated GSC CSV.
